...
目前建立的是 Binary Network Intrusion Detection System。

輸入 UNSW-NB15 的 Network Traffic Features，透過 Machine Learning 判斷一筆 Network Traffic 為：

Normal 或 Attack。

---

## Q2：為什麼不用 attack_cat 當 Feature？

因為 attack_cat 本身包含攻擊類型資訊。

Binary Classification 的 Label 又是在判斷 Normal / Attack，因此使用 attack_cat 可能造成 Data Leakage。

---

## Q3：為什麼原本 42 Features 最後變 194？

因為 `proto`、`service`、`state` 是 Categorical Features。

使用 One-Hot Encoding 後，一個類別型 Feature 會依照不同 Category 展開成多個 Binary Features。

---

## Q4：為什麼 Logistic Regression 要 StandardScaler？

Logistic Regression 對不同 Feature 的數值尺度較敏感，因此 Numerical Features 進行 Standardization。

---

## Q5：為什麼 Random Forest 不需要 StandardScaler？

Random Forest 是 Tree-based Model。

Decision Tree 主要依照 Feature 的切分點進行決策，因此通常不受 Feature Scale 影響。
...
尤其如果 Attack 被判斷為 Normal，就會形成 False Negative，代表 IDS 漏掉真正的攻擊。

---

## Q7：目前哪個模型比較好？

Day 1 的實驗中 Random Forest 較好。
...
因此 Random Forest 同時降低誤報與漏報。

---

## Q8：什麼是 Baseline？

Baseline 是用來當作後續模型比較基準的初始模型。

本專題首先使用 Logistic Regression 建立 Baseline，再與 Random Forest 等更複雜的模型比較。

---

## Q9：Feature Importance 代表什麼？

代表目前 Random Forest 在進行分類決策時，各 Feature 的相對重要程度。

但 Feature Importance 不代表因果關係。
